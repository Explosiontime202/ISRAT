...
use gtk4::{
    traits::WidgetExt, traits::*, Box as GtkBox, CenterBox, DropDown, Expression, Inhibit, Label,
    StringList, Switch, Widget,
};
...
fn create_auto_save_interval_setting() -> Widget {
    ...
    create_settings_selector(
        ...
        |_, sel_idx, sel_option| {
            println!("Auto-save interval: Selected {sel_option} at idx {sel_idx}",)
        },
    )
}
...
fn create_new_rules_switch() -> Widget {
    ...
    create_settings_switch("Use new rules", |_, state| {
...
fn create_fullscreen_switch() -> Widget {
    ...
    create_settings_switch("Fullscreen", |_, state| {
...
fn create_settings_selector<F: Fn(&DropDown, u32, String) + 'static>(
    text: &str,
    options: &[&str],
    callback: F,
) -> Widget {
