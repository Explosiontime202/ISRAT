...
pub fn build(ui: &Ui, program_state: &mut ProgramState, menu_bar_height: f32) {
    ...
    ChildWindow::new("##erg_view_screen")
        ...
        .build(ui, || {
            ...
            //if ui.begin_combo("##select", preview_value)
            ...
            if let Some(_table_token) = ui.begin_table_with_flags(
                ...
                ui.set_cursor_pos([
                    ui.cursor_pos()[0]
                        + (ui.content_region_avail()[0] - ui.calc_text_size("Stock Points")[0])
                            / 2.0,
                    cursor_y_pos,
                ]);
                ui.text("Stock Points");

                // TODO: Use value from selector
                let group_idx = 0;
                    ...
                    .for_each(|(place_idx, entry)| {
                        ...
                        ui.set_cursor_pos([
                            ui.cursor_pos()[0]
                                + (ui.content_region_avail()[0]
                                    - ui.calc_text_size(
                                        &data.teams.as_ref().unwrap()[group_idx][entry.team_idx]
                                            .name,
                                    )[0])
                                    / 2.0,
                            cursor_y_pos,
